...
graph TD
    ...
    subgraph "感知层"
        C1[SSFR纹理先验] --> C2[亮度引导图]
        C3[Doubao掩码监督] --> C4[ROI坐标导引]
    end
...
| **物理感知** | SSFR + 亮度门控 | 高频纹理与局部亮度异常 | 局部伪造定位 |
| **模型训练** | V14融合架构 | CLIP + ConvNeXt + FLH | 多尺度特征融合 |
...
### (三) 定位监督准备（在线亮度图 + 可选掩码同步）

#### 技术原理：亮度引导与局部编辑掩码
**核心功能**：当前主线不再预生成 TruFor 热力图，而是在训练阶段在线生成亮度引导图，并仅对带有局部编辑掩码的样本计算多尺度定位损失。

#### 工程机制
```
原始图像 → 在线亮度图生成 → 掩码对齐检查 → 多尺度定位监督 → ROI坐标导引
        ↓              ↓              ↓               ↓              ↓
    数据加载      亮度异常提示      文件映射校验     32/64/128损失     热力图输出
```

#### 执行命令
```bash
# 如更新了 doubao 局部编辑数据，按需同步掩码
...
localization_supervision = {
        "mask_alignment": "文件名与标注一一对应",
        "online_guidance": "亮度图训练时即时生成",
        "supervision_levels": [32, 64, 128],
        "masked_only_loss": True,
        "training_mode": "freeze_loc_epochs + multiscale dice"
}
```

### (四) V14多尺度定位融合训练 (5_train_model_v11.py)

#### 策略优势：智能重采样系统
**核心创新**：采用**智能重采样（Smart Resampling）**与 `freeze_loc_epochs` 策略，在大数据基座上先稳定分类优化，再逐步解冻定位分支，强化模型对微小局部重绘的捕捉能力。

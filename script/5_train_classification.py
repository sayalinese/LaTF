...
def train_one_epoch(model, loader, optimizer, scaler, criterion, device, epoch, writer, args):
    ...
    for i, batch in enumerate(loader):
        ...
        doubao_fake_w = getattr(args, 'doubao_fake_weight', 3.0)
        for _j, (_p, _lbl) in enumerate(zip(img_paths, labels.cpu().tolist())):
            if 'doubao' in _p.lower().replace('\\', '/'):
                sample_weights[_j] = doubao_real_w if _lbl == 0 else doubao_fake_w
...
def main():
    ...
    parser.add_argument('--train_file', type=str, default=os.getenv('TRAIN_FILE', 'annotation/train_v2.txt'), help='Train txt list')
    parser.add_argument('--val_file', type=str, default=os.getenv('VAL_FILE', 'annotation/val_v2.txt'), help='Val txt list')
    parser.add_argument('--map_file', type=str, default=os.getenv('MAP_FILE', 'dift.pt/ann.txt'), help='Precomputed LaRE features path')
    ...
    parser.add_argument('--doubao_fake_weight', type=float,
                        default=float(os.getenv('DOUBAO_FAKE_WEIGHT', '3.0')),
                        help='豆包假图(label=1)被误判时的损失惩罚倍率')
    ...
    if not args.resume and (out_dir / 'latest.pth').exists():
        args.resume = str(out_dir / 'latest.pth')
    ...
    elif args.freeze_loc_epochs > 0 and start_epoch <= args.freeze_loc_epochs:
    ...
    if checkpoint is not None:
        ...
        if 'history' in checkpoint and checkpoint['history'] is not None:
            ...
            logger.info(f"History restored with {len(history['epoch'])} records.")
    ...
    # Track statistics
    # Track statistics (history is already initialized above)
    ...
    try:
        ...
        logger.info('User stopped training. Saving history...')

    writer.close()
    logger.info('Training process finished or interrupted.')

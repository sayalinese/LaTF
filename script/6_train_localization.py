...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        # 裁剪 cross entropy 以防 AMP 环境下 exp(-ce) 导致梯度爆炸
        ce_clamped = torch.clamp(ce, min=1e-5, max=15.0)
...
def train_one_epoch(model, loader, optimizer, criterion, scaler, device, epoch, freeze_encoder):
    ...
    for images, masks in progress:
        ...
        with autocast(device_type=device, enabled=scaler.is_enabled()):
            ...
            # 限制 logits 幅度，避免 AMP 和 FocalLoss 中 exp(-ce) 导致的数值溢出 (NaN)
...
def main():
    ...
    start_epoch = 1
    best_dice = 0.0
    latest_pth = Path(args.out_dir) / 'latest.pth'
    if latest_pth.exists():
        print(f"Auto-resuming from {latest_pth}...")
        checkpoint = torch.load(latest_pth, map_location=device)
        model.load_state_dict(checkpoint['state_dict'] if 'state_dict' in checkpoint else checkpoint)
        if 'epoch' in checkpoint:
            start_epoch = checkpoint['epoch'] + 1
            if 'best_dice' in checkpoint: best_dice = checkpoint['best_dice']
            if 'optimizer' in checkpoint: optimizer.load_state_dict(checkpoint['optimizer'])
            if 'scheduler' in checkpoint: scheduler.load_state_dict(checkpoint['scheduler'])
            if 'scaler' in checkpoint and scaler is not None: scaler.load_state_dict(checkpoint['scaler'])
            if 'patience_counter' in checkpoint: globals()['patience_counter'] = checkpoint['patience_counter']
